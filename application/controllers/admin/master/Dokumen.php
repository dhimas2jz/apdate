...
class Dokumen extends CI_Controller {
	...
	public function __Construct() {
		...
		$this->cekLogin();
		$this->own_link = admin_url('master/dokumen');
		$this->load->model('master/Dokumen_model');
		$this->table = "mt_dokumen";
		$this->judul = "Dokumen";
	}
	...
	public function index() {
		...
		$this->template->_v('master/dokumen/index', $data);
	}
	...
	public function datatables() {
        ...
        foreach ($list as $field) {
            ...
            $row_menu[] = '<a href="'.base_url($field->file).'" target="_blank" class="btn btn-success mb-2">Lihat File</a>';
	...
	public function create() {
		...
		$this->template->_v('master/dokumen/form', $data);
	}

	public function edit($id) {
		$id = (int) $id;
		$this->privilege('is_update');
        $model = $this->Dokumen_model->find($id);
        if (empty($model)) {
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST') {
			...
			if (isset($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
				...
				if ($this->upload->do_upload('file')) {
					...
					$post_data['file'] = 'upload/dokumen/' . $upload_data['file_name'];
				} else {
	...
	private function validation($post_data) {
		...
		// if (!empty($id)) {
		// 	$data = $this->Dokumen_model->find($id);
		// 	if (empty($data)) {
		// 		$this->session->set_flashdata('error', "Data not found");
	    //     	return redirect($this->own_link);
	    //     }
